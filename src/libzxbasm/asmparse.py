...
def p_def_label(p):
    ...
    """
    p[0] = None
    __DEBUG__("Declaring '%s%s' in %i" % (common.NAMESPACE, p[1], p.lineno(1)))
...
def p_asm_label(p):
    """ asm : ID
    """
    __DEBUG__("Declaring '%s%s' (value %04Xh) in %i" % (common.NAMESPACE, p[1], MEMORY.org, p.lineno(1)))
...
def p_namespace(p):
    ...
    common.NAMESPACE = normalize_namespace(p[2])
    __DEBUG__('Setting namespace to ' + (common.NAMESPACE.rstrip(DOT) or DOT), level=1)
